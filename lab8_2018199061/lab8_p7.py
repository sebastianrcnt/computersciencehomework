...
def race():
    """
    Starts the race and returns the winner index

    :return (int): winner
    """
    # declare that horse_images and banner_images refer to variables in the global scope.
    global horse_images, banner_images
...
# keep asking for input until user presses 'q'
ans = input("Press 'q' to quit:")
winners = []  # list of winners(in indexes 0~9)
while ans != 'q':
    winners.append(race())  # run a race and append the winner horse index to winners list
    turtle.clearscreen()  # clear the screen and wait for user input
    ans = input("Press 'q' to quit:")

# print how many times did each horse win
